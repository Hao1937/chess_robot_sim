...
|---|---|---|---|---|
| `parse_command(command_text)` | `src/interaction/cli.py` | `"A1 B1"` / `"reset"` / `"hand_on"` / `"obstacle_mode 2"` | `MoveCommand` | 解析基础命令和障碍预设模式 |`n| `parse_chinese_move(text, board, side="red")` | `src/interaction/chinese_notation.py` | `"车二平七"` / `"炮五进四"`、棋盘状态 | `MoveCommand` | 只解析红方简化中文记谱；同路同类歧义时要求 `前` / `后` |
...
| `build_scene(config, obstacle_mode)` | `src/simulation/scene_builder.py` | 配置、障碍预设模式 | `SceneHandle` | 返回棋盘、棋子、预设圆柱障碍物 id |
| `build_obstacle_preset(obstacle_mode, config)` | `src/simulation/scene_builder.py` | `mode_1` / `mode_2` / `mode_3` / `none` | `list[Obstacle]` | 生成 2-3 个竖直圆柱障碍物 |
