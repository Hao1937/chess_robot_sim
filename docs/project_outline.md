...
- 不做真实视觉识别，不从摄像头识别棋盘。
- 不做真实夹爪动力学，棋子抓取用虚拟磁吸附或 suction-like attachment。
...
6. 用户可以触发 `hand_on`，模拟人手进入棋盘区域，机械臂进入暂停或安全模式。
...
- `src/interaction/`：A 负责，处理用户输入、棋盘状态、走法规则、reset、GUI 命令。
...
其他棋子可以只做基础合法性检查，例如起点有棋子、目标格不是己方棋子。
...
C 会把棋子、障碍柱、人手安全区都变成带安全半径的障碍物。路径接近这些障碍时，轨迹 planner 进入更保守模式。
...
4. 如果机械臂正在执行任务，系统进入暂停或 Safe mode。
5. D 记录暂停时间、执行状态或安全模式切换。
...
3. 展示绕障：有障碍时路径变化。
4. 展示 Fast/Safe mode：靠近棋子或障碍物时减速。
5. 展示 reset。
6. 展示 hand_on / hand_off。
7. 展示误差曲线和结果表。
