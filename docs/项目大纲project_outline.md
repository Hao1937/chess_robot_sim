...
9. 用户可以切换 `obstacle_mode 1/2/3`，场景中出现不同位置的竖直圆柱障碍，用来测试避障规划。
10. 用户可以输入少量红方中文记谱，例如 `车二平七`、`炮五进四`，系统把它转换成坐标走法。
...
10. D 的 `summarize_execution()` 和 `plot_results()` 输出误差和曲线数据。

如果用户输入的是 `车二平七` 或 `炮五进四`，A 先用 `parse_chinese_move()` 根据当前棋盘找到对应红方棋子，再转换成同样的 `MoveCommand`，后续流程不变。

## 6.1 持续交互模式

除了一次性 demo，项目还提供长期交互入口：

```bash
python main.py --interactive
```

interactive 模式只在开始时初始化一次 `BoardState`、robot 和 scene。之后主循环不断调用 A 的 `poll_gui_command()` 获取用户输入，每次拿到一条 `MoveCommand` 后跑一轮 A/B/C/D pipeline，成功后更新内存中的棋盘状态，然后继续等待下一条命令。

这个设计保证 PyBullet 可视化窗口和棋盘状态不是一次性的。A 的 GUI 只负责产生命令，C 的 session 负责持续调度，B 的仿真场景负责持续显示和更新。
...
11. 切换不同 obstacle mode 后，展示路径和避障距离变化。
12. 输入 `车二平七` 或 `炮五进四` 后，系统能转换为坐标走法并执行。
